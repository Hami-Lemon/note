...
会列出指定的每一个远程仓库的简写（可使用-v选项显示对应的URL)
...
会访问远程仓库，拉取所有还没有的数据，执行完成后，将会拥有那个远程仓库的所有的引用。
...
轻量标签只是某个特定分支的引用，而附注标签是一个完整对象，包含打标签者的名字，邮箱等。
...
只会删除本地仓库的标签，不会删除远程仓库的标签，需使用`git push origin --delete <tagname>`删除。